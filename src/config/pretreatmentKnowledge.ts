...
export const pretreatmentKnowledge = [
  {
    ...
    content: `
      ...
      <p><strong>中文文档预处理流程：</strong>确定索引的文档单位（document unit）、词条化、去停用词、<strong>去标点。</strong>（说明：一般的检索系统中默认每篇文档就是固定的检索单位）</p>
    `
  },
  {
    title: '分词',
    content: `<p><strong>中文分词</strong></p>
              ...
              <p>现有的分词算法可分为三大类：基于<strong>字符串匹配</strong>的分词方法、基于理解的分词方法和基于统计的分词方法。</p>
              <p>常用的中文分词器有：<strong>标准分词器、空格分词器、简单分词器、二分法分词器</strong></p>
              <p><strong>字符匹配</strong></p>
              <p style="text-indent: 2em;">
                这种方法又叫做<strong>机械分词方法</strong>，它是按照一定的策略将待分析的汉字串与一个&ldquo;充分大的&rdquo;机器词典中的词条进行配，若在词典中找到某个字符串，则匹配成功（识别出一个词）。按照扫描方向的不同，串匹配分词方法可以分为正向匹配和逆向匹配；按照不同长度优先匹配的情况，可以分为最大（最长）匹配和最小（最短）匹配；常用的几种机械分词方法如下：
              </p>
              <p>1）<strong>正向最大匹配法</strong>（由左到右的方向）；</p>
              <p>2）<strong>逆向最大匹配法</strong>（由右到左的方向）；</p>
              <p>3）<strong>最少切分</strong>（使每一句中切出的词数最小）；</p>
              <p>4）<strong>双向最大匹配法</strong>（进行由左到右、由右到左两次扫描）</p>
              <p><strong>理解法</strong></p>
              <p style="text-indent: 2em;">
                这种分词方法是通过让计算机模拟人对句子的理解，达到识别词的效果。其基本思想就是在分词的同时进行句法、语义分析，利用句法信息和语义信息来处理歧义现象。它通常包括三个部分：<strong>分词子系统、句法语义子系统、总控部分。</strong>在总控部分的协调下，分词子系统可以获得有关词、句子等的句法和语义信息来对分词歧义进行判断，即它模拟了人对句子的理解过程。这种分词方法需要使用大量的语言知识和信息。由于汉语语言知识的笼统、复杂性，难以将各种语言信息组织成机器可直接读取的形式，因此目前基于理解的分词系统还处在试验阶段。
              </p>
  ...
  },
  {
    ...
    content: `<p><strong>概念：</strong></p>
              ...
              <p>
                2、文本中<strong>出现频率</strong>很高，但实际意义又不大的词。这一类主要包括了语气助词、副词、介词、连词等，通常自身并无明确意义，只有将其放入一个完整的句子中才有一定作用的词语。如常见的&ldquo;的&rdquo;、&ldquo;在&rdquo;、&ldquo;和&rdquo;、&ldquo;接着&rdquo;之类，比如&ldquo;SEO研究院是原创的SEO博客&rdquo;这句话中的&ldquo;是&rdquo;、&ldquo;的&rdquo;就是两个停用词。
              </p>
