...
export const entryKnowledge = [
  {
    ...
    content: `<p style="text-indent: 2em;">
                <strong>信息检索(Information Retrieval)</strong>是从大规模非结构化数据的集合（通常保存在计算机上）中找出满足用户需求的资料（通常是文档）的过程。
                <strong>搜索引擎</strong>是指根据一定的策略、运用特定的计算机程序从互联网上采集信息，在对信息进行组织和处理后，为用户提供检索服务，将检索的相关信息展示给用户的系统。搜索引擎是工作于互联网上的一门检索技术，它旨在提高人们获取搜集信息的速度，为人们提供更好的网络使用环境。网络搜索引擎（谷歌、百度、中国知网、Web of Science、微信搜一搜等）是获取各类互联网大数据的门户，是各类互联网应用的基石。掌握网络搜索引擎构架、了解信息检索技术、掌握构建信息检索系统的方法是信息管理与信息系统专业学生的所必须的基本素养。
              </p>
              ...
              <p style="text-indent: 2em;font-weight: bold">
                本实验流程为：构建索引器（构建预处理器、构建倒排索引表）、构建检索器（构建布尔模型、构建向量空间模型、构建概率检索模型、构建语言模型）、分析检索模型性能、仿真搜索引擎。本实验要求通过完成<strong>文档预处理、</strong>构建倒排索引、构建检索模型三个子实验设计一个搜索引擎，并对自己搭建的搜索引擎性能的进行评价。
              </p>`
  },
  {
    title: '相关技术',
